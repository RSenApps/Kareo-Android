 ...
 * 
 */
public class MyTitleFlowIndicator extends TextView implements FlowIndicator {
	//modded to not load all dates
	...
	private int actualPosition = 1000000000;
	...
	private Path footerLinePath;
	private Path footerTrianglePath;
	private Typeface typeface;
	private int width = 0;
	/**
	 * Default constructor
	 */
	public MyTitleFlowIndicator(Context context) {
		super(context);
		Display display = ((scheduleViewActivity) context).getWindowManager().getDefaultDisplay();
	 ...
	 * @param attrs
	 */
	public MyTitleFlowIndicator(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	private void drawCanvas(Canvas canvas)
	{
		...
		footerLinePath = new Path();
		int coordY = getHeight() - 1;
		coordY -= (footerLineHeight % 2 == 1) ? footerLineHeight / 2 : footerLineHeight / 2 - 1;
		footerLinePath.moveTo(0, coordY);
		footerLinePath.lineTo(getWidth(), coordY);
		footerLinePath.close();
		footerTrianglePath = new Path();
		footerTrianglePath.moveTo(getWidth() / 2, getHeight() - footerLineHeight - footerTriangleHeight);
		footerTrianglePath.lineTo(getWidth() / 2 + footerTriangleHeight, getHeight() - footerLineHeight);
		footerTrianglePath.lineTo(getWidth() / 2 - footerTriangleHeight, getHeight() - footerLineHeight);
		footerTrianglePath.close();
		canvas.drawPath(footerLinePath, paintFooterLine);
		canvas.drawPath(footerTrianglePath, paintFooterTriangle);
	}
	...
	private ArrayList<Rect> calculateAllBounds(Paint paint) {
		...
		for (int iLoop = 0; iLoop < count; iLoop++) {
			...
			bounds.left = (getWidth() / 2) - (w / 2) - currentScroll + ((iLoop - 3) * getWidth());
	 ...
	 * @return
	 */
	private String getTitle(int posWithinSeven) {
		// Set the default title
		int pos = actualPosition -3 + posWithinSeven; // 1000000000 is days depth 3 is to compensate for 7 loaded views
	...
	public void onScrolled(int h, int v, int oldh, int oldv) {
		...
		currentScroll = h - actualPosition * width;
		prepareDraw();
	}
	...
	public void onSwitched(View view, int position) {
		...
		currentScroll += (actualPosition-position) *width;
	...
	public void setViewFlow(ViewFlow view) {
		actualPosition = view.getSelectedItemPosition();
		prepareDraw();
	}
